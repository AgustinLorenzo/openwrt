...
( \
  echo 'src/gz %d_core https://openwrtdata.agustinls.com/nss-wifi-mesh/$(shell date +"%Y%m%d")/packages'; \
  $(strip $(if $(CONFIG_PER_FEED_REPO), \
...
( \
  echo 'https://openwrtdata.agustinls.com/nss-wifi-mesh/$(shell date +"%Y%m%d")/packages/packages.adb'; \
  $(strip $(if $(CONFIG_PER_FEED_REPO), \
